...
(() => {
  ...
  const updatePanel = () => {
    ...
    mainPanel.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; padding-bottom: 10px; border-bottom: 1px solid rgba(255,255,255,0.2);">
        <div style="font-size: 24px; cursor: move; width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; border-radius: 50%; background: ${isRunning ? 'rgba(255,50,50,0.3)' : 'rgba(0,255,0,0.3)'};">
          ${isRunning ? "⏹️" : "▶️"}
        </div>
  ...
  const addTradeLog = (message, isWin) => {
    const now = new Date();
    const timeString = now.toTimeString().substring(0, 8);
    ...
    logEntry.innerHTML = `
      <div style="width: 20px; height: 20px; border-radius: 50%; background: ${isWin ? 'rgba(0,255,0,0.2)' : 'rgba(255,0,0,0.2)'}; display: flex; align-items: center; justify-content: center; margin-right: 8px;">
