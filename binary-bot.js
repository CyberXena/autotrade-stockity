...
try {
  ...
  let autostartTriggered = false; // Tambahan flag supaya trigger hanya sekali
  ...
  // ... kode state, saveState, resetState, calculateNextStake, dst ...
  ...
  function tryAutoStartBot() {
    ...
    // Debug log
    console.log("[AutoStart] Checking:", hour, minute, autostartHour, autostartMinute, state.isRunning, autostartTriggered);

    if (!state.isRunning && !autostartTriggered &&
        hour === autostartHour && minute === autostartMinute && !checkTargetProfit()) {
      ...
      console.log("[AutoStart] Bot started automatically!");
    }
    // Reset flag jika jam/minut sudah lewat
    if (hour !== autostartHour || minute !== autostartMinute) {
